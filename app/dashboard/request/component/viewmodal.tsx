...
export default function EnhancedRequestModal({
  ...
  const handleAccept = async () => {
    await updateRequestStatus(selectedRequest._id, true);

    onClose();
  };

  const handleReject = async () => {
    await updateRequestStatus(selectedRequest._id, false);
    onClose();
  };
  return (
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden shadow-2xl">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-blue-100 text-sm">
                Review the request information carefully
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="overflow-y-auto max-h-[calc(95vh-200px)] p-6">
          ...
          <div className="flex items-center gap-4 mb-6">
            {selectedRequest?.profilePicture != null ? (
              <img
          ...
          <div className="mb-6 flex flex-wrap gap-2">
            <span
              className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold ${
                selectedRequest.profileStatus === "Pending"
                  ? "bg-yellow-100 text-yellow-800"
                  : selectedRequest.profileStatus === "Approved"
                    ? "bg-green-100 text-green-800"
                    : "bg-red-100 text-red-800"
              }`}
            >
              {selectedRequest.profileStatus}
            </span>
